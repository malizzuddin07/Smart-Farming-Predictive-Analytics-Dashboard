...
try:
    ...
    if os.path.exists(key_path):
        ...
        logger.warning(
            f"GEE Key file not found at {key_path}. Trying default credentials..."
        )
...
def get_db_connection():
    try:
        ...
        if os.path.exists(ssl_ca):
            ...
            logger.warning(
                "SSL CA cert not found. Attempting insecure connection (okay for localhost, bad for cloud)."
            )
...
def check_for_anomalies(raw_inputs):
    ...
    if ndvi < 0.2:
        anomalies.append(
            "Very low NDVI detected. Field may be bare or facing severe stress."
        )
    ph = float(raw_inputs.get("SOIL_pH") or 0.0)
    if ph < 5.0 or ph > 7.5:
        anomalies.append(f"Soil pH ({ph:.1f}) is outside the optimal 5.5-7.0 range.")
    tmin = float(raw_inputs.get("TMIN_All") or 0)
    tmax = float(raw_inputs.get("TMAX_All") or 0)
    if tmin < 15 or tmax > 40:
        anomalies.append("Extreme temperature forecast detected.")
    return anomalies
    ...
    try:
        ...
        try:
            s2 = (
                ...
                .filterBounds(area)
                .filterDate(start_date_recent, end_date)
            ...
            recent_image = (
                s2.map(calculate_ndvi).sort("system:time_start", False).first()
            )
            if recent_image:
                val = recent_image.reduceRegion(
                    reducer=ee.Reducer.mean(), geometry=area, scale=10
                ).get("NDVI")
                if val.getInfo():
                    result["latest_ndvi"] = float(val.getInfo())
        except Exception:
            pass

        # Soil
        try:
            ph_img = ee.Image("projects/soilgrids-isric/phh2o_mean").select(
                "phh2o_0-5cm_mean"
            )
            cec_img = ee.Image("projects/soilgrids-isric/cec_mean").select(
                "cec_0-5cm_mean"
            )
            oc_img = ee.Image("projects/soilgrids-isric/soc_mean").select(
                "soc_0-5cm_mean"
            )

            # Reduce regions
            data = (
                ee.Image.cat([ph_img, cec_img, oc_img])
                .reduceRegion(
                    reducer=ee.Reducer.firstNonNull(), geometry=point, scale=250
                )
                .getInfo()
            )

            if data:
                if "phh2o_0-5cm_mean" in data:
                    result["soil_ph"] = float(data["phh2o_0-5cm_mean"]) / 10.0
                if "cec_0-5cm_mean" in data:
                    result["soil_cec"] = float(data["cec_0-5cm_mean"]) / 10.0
                if "soc_0-5cm_mean" in data:
                    result["soil_oc"] = float(data["soc_0-5cm_mean"]) / 100.0

        except Exception as soil_err:
            logger.error(f"GEE Err Soil: {soil_err}")
    except Exception as e:
        logger.error(f"GEE General Error: {e}")
    return result
...
def get_historical_srad(lon, lat, days_back=365):
    ...
    try:
        ...
        logger.error(f"SRAD History Error: {e}")
...
def get_historical_or_forecast_weather(lon, lat, target_date):
    ...
    try:
        ...
        if days_diff < 0:  # Past
            ...
            resp.raise_for_status()
            day = (
                resp.json().get("forecast", {}).get("forecastday", [])[0].get("day", {})
            )
            tmin = day.get("mintemp_c", tmin)
            tmax = day.get("maxtemp_c", tmax)
            rain = day.get("totalprecip_mm", rain)
        else:  # Future/Today
            url = f"http://api.weatherapi.com/v1/forecast.json?key={key}&q={lat},{lon}&days=3&aqi=no&alerts=no"
            resp = requests.get(url, timeout=10)
            resp.raise_for_status()
...
def normalize_value(value, feature_name):
    ...
    return (
        max(0.0, min(1.0, (value - p["min"]) / (p["max"] - p["min"])))
        if p["max"] != p["min"]
        else 0.5
    )
...
try:
    ...
    X = df[features]
    y = pd.to_numeric(df["Hasil Kasar"], errors="coerce")
    full_df = X.join(y).dropna()
    model = Ridge()
    model.fit(full_df[features], full_df["Hasil Kasar"])
...
def register_user():
    ...
    try:
        cur.execute(
            "INSERT INTO users (fullname,ic,state,phone,password) VALUES (%s,%s,%s,%s,%s)",
        ...
        flash("Registration failed. IC may exist.", "error")
        logger.error(e)
        return redirect(url_for("register_page"))
    finally:
        cur.close()
...
def dashboard_home():
    ...
    if conn:
        cur = conn.cursor(dictionary=True)
        cur.execute(
            "SELECT * FROM planting_sessions WHERE user_ic=%s ORDER BY planting_date DESC",
            (session["ic"],),
        )
...
def create_session():
    ...
    try:
        ...
        key = os.environ.get("WEATHERAPI_KEY")
        geo_url = (
            f"http://api.weatherapi.com/v1/search.json?key={key}&q={f['location']}"
        )
        ...
        raw = {
            ...
            "SOIL_CEC (meq/100)": (
                float(f["SOIL_CEC"]) if f.get("SOIL_CEC") else gee["soil_cec"]
            ),
        ...
        input_vector = pd.DataFrame(
            [{k: normalize_value(v, k) for k, v in raw.items()}]
        )[features]
        ...
        # Get template offset
        cur.execute(
            "SELECT harvest_day_offset FROM workflow_templates WHERE id=%s",
            (f["template_id"],),
        )
        ...
        # Create Tasks
        cur.execute(
            "SELECT task_name, days_offset FROM workflow_template_steps WHERE template_id=%s",
            (f["template_id"],),
        )
        ...
        for step in steps:
            ...
            status = (
                ...
                else ("in_process" if t_date == date.today() else "soon")
            )
        ...
        return redirect(url_for("session_dashboard", session_id=sess_id))
...
def session_dashboard(session_id):
    ...
    # Fetch Session Data
    cur.execute(
        "SELECT * FROM planting_sessions WHERE session_id=%s AND user_ic=%s",
        (session_id, session["ic"]),
    )
    ...
    if hist:
        ...
        input_breakdown["normalized"] = {
            k: f"{normalize_value(v, k):.2f}" for k, v in raw.items() if v is not None
        }
...
def delete_session(session_id):
    ...
    cur = conn.cursor()
    cur.execute(
        "DELETE FROM planting_sessions WHERE session_id=%s AND user_ic=%s",
        (session_id, session["ic"]),
    )
    conn.commit()
    cur.close()
...
def update_session(session_id):
    ...
    cur.execute(
        "UPDATE planting_sessions SET session_name=%s, field_size=%s, predicted_total_yield=predicted_yield_per_ha*%s WHERE session_id=%s",
...
def change_password():
    ...
    try:
        ...
        if user and user["password"] == current_pass_input:
            ...
            cur.execute(
                "UPDATE users SET password=%s WHERE ic=%s", (new_pass, session["ic"])
            )
            conn.commit()
